...
export const Header = () => {
  ...
  return (
    <Flex as="header" alignItems="center" justifyContent="space-between">
      ...
      {openAccountModal ? (
        <Box as="button" textTransform="uppercase" onClick={openAccountModal}>
          {ensName || trimAddress(address || "0x0000000")}
        </Box>
      ) : (
        <Box as="button" textTransform="uppercase" onClick={openConnectModal}>
          Not connected
        </Box>
      )}
    </Flex>
